/**
 * nexMultipart parser implementation
 */
...
import type { Field, Limits, FileInfo } from "./types.js";
import { asFieldName, asFileName, asMimeType, asBytes } from "./types.js";
...
export class NativeMultipartParser {
  ...
  constructor(options: ParserOptions = {}) {
    this.limits = {
      fieldNameSize: options.limits?.fieldNameSize || asBytes(100),
      fieldSize: options.limits?.fieldSize || asBytes(1024 * 1024), // 1MB
      fields: options.limits?.fields || Infinity,
      fileSize: options.limits?.fileSize || asBytes(Infinity),
  ...
  async parse(
    req: IncomingMessage,
  ): Promise<{ fields: Field[]; files: FileInfo[] }> {
  ...
  private async parseWithAsyncContext(
    req: IncomingMessage,
    boundary: string,
  ): Promise<{ fields: Field[]; files: FileInfo[] }> {
    const chunks: Buffer[] = [];
    ...
    for await (const chunk of req) {
      ...
      if (
        totalSize >
        this.limits.fieldSize * this.limits.fields +
          this.limits.fileSize * this.limits.files
      ) {
    ...
    for (const part of parts) {
      ...
      if (filenameMatch) {
        ...
        files.push({
          ...
          originalname: asFileName(filename),
          encoding: "7bit",
          mimetype: asMimeType(contentType),
          stream,
        });
      } else {
        // It's a text field
  ...
  private parsePartHeaders(part: Buffer): {
