...
import { asBytes, asFileName } from "../types.js";
import { ValidateUpload } from "../decorators/validate.js";
...
export class DiskStorage implements StorageEngine {
  ...
  private filename:
    ...
    | ((
        req: Request,
        file: File,
        callback: (error: Error | null, filename: FileName) => void,
      ) => void);
  ...
  private defaultFilename(
    ...
    callback(null, asFileName(uniqueName));
  }
  ...
  private getFilename(
    ...
    if (typeof this.filename === "function") {
      ...
      callback(null, asFileName(this.filename as string));
    }
  }
}
