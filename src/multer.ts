...
export class Multer {
  ...
  constructor(options: Options = {}) {
    ...
    this.storage =
      options.storage ||
      new DiskStorage({ destination: options.dest || "./uploads" });
  ...
  single(fieldname: string): RequestHandler {
    return this.createMiddleware("single", asFieldName(fieldname));
  }
  ...
  array(fieldname: string, maxCount?: number): RequestHandler {
    return this.createMiddleware("array", asFieldName(fieldname), maxCount);
  }
      ...
      const processedFile = await new Promise<File>((resolve, reject) => {
        this.storage._handleFile({} as Request, fileInfo, (error, info) => {
          ...
          const file: File = {
            ...
            size: asBytes(info?.size || 0),
            destination: info?.destination,
            filename: asFileName(info?.filename || ""),
            path: info?.path,
            buffer: info?.buffer,
            location: info?.location,
            etag: info?.etag,
            bucket: info?.bucket ? asBucketName(info.bucket) : undefined,
            key: info?.key ? asStorageKey(info.key) : undefined,
          };
    ...
    switch (type) {
      ...
      case "fields":
        if (Array.isArray(fieldname)) {
          const fieldSpec = fieldname.find(
            (f) => f.name === fileInfo.fieldname,
          );
          if (!fieldSpec) {
            throw new MulterError("LIMIT_UNEXPECTED_FILE", fileInfo.fieldname);
          }
